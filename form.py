...
from workers import DF, ROR, RP_MSK, RP_SPB, ISK
from create import path_to_folder, destination_folder, create_task_folder, start_update
from base import create_db, get_form_data, get_non_typical_works, update_form_data, update_non_typical_works, delete_non_typical_works_by_user_id
...
def start(message):
    ...
    update_form_data(message.chat.id, {'user_id' : message.chat.id})
...
def handle_button_press(call):
    ...
    try:
        if len(call.data.split(',')) > 1:
            update_form_data(user_id, {'name' :  call.data.split(',')[0]})

            update_form_data(user_id, {'number' :  call.data.split(',')[1]})

            update_form_data(user_id, {'adr' :  values.get(get_form_data(user_id) ['name'])[get_form_data(user_id)['number']]})
            update_form_data(user_id, {'[филиал]' :  get_form_data(user_id)['adr'].split(' ')[1]})
        ...
        if call.data == 'begin_apo':
            ...
            bot.send_message(call.message.chat.id, 'Выбран объект: ' + get_form_data(user_id)['adr'] + '. Выберите тип фото:', reply_markup=keyboard_folder)
   
        elif call.data == 'back_to_main':
            bot.send_message(call.message.chat.id, 'Выберите действие по объекту: ' + get_form_data(user_id)['adr'] + '.', reply_markup=keyboard_2)
        ...
        if folder is not None:
            update_form_data(user_id, {'objects_path' :  path_to_folder  + get_form_data(user_id)['adr'] + '\\Фото\\' + folder})
            bot.send_message(call.message.chat.id, 'Отправьте фото! Путь к папке: ' + get_form_data(user_id)['objects_path'])
...
def handle_photo(message):
    ...
    filepath = os.path.join(get_form_data(message.chat.id)['objects_path'], file_info.file_path.split('/')[-1])
...
def start_form(message):
    ...
    update_form_data(user_id, {'[адрес]' :  ' '.join(get_form_data(user_id)['adr'].split(' ')[1:])})
    # Находим полние имя РП
    for text in (RP_MSK + RP_SPB):
        if get_form_data(user_id)['name'] in text:
            update_form_data(user_id, {'[рп]' :  text})
    if get_form_data(user_id)['филиал'] == 'МСК':
        update_form_data(user_id, {'[ИСК]' :  ISK[0]})
        update_form_data(user_id, {'[РОР]' :  ROR[0]})
    elif get_form_data(user_id)['филиал'] == 'СПБ':
        update_form_data(user_id, {'[ИСК]' :  ISK[1]})
        update_form_data(user_id, {'[РОР]' :  ROR[1]})
...
def process_area(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[площадь_помещения]' :  'Пропущено'})
        ask_floor(message)
        return
    update_form_data(message.chat.id, {'[площадь_помещения]' :  message.text})
    ask_floor(message)
...
def process_floor(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[этаж]' :  'Пропущено'})
        ask_building_floors(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[этаж]' : None})
        start_form(message)
        return
    update_form_data(message.chat.id, {'[этаж]' :  message.text})
    ask_building_floors(message)
...
def process_building_floors(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[этажность]' :  'Пропущено'})
        askobjecttype(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[этажность]' : None})
        ask_floor(message)
        return
    update_form_data(message.chat.id, {'[этажность]' :  message.text})
    askobjecttype(message)
...
def processobjecttype(message): 
    if message.text == 'Пропустить': 
        update_form_data(message.chat.id, {'[тип_объекта]' :  'Пропущено'}) 
        ask_basement_use_2(message) 
        return 
    elif message.text == 'Назад': 
        update_form_data(message.chat.id, {'[тип_объекта]' : None})
        ask_building_floors(message) 
        return 
    update_form_data(message.chat.id, {'[тип_объекта]' :  message.text}) 
    ask_basement_use_2(message)
...
def process_basement_use_2(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[использование_подвала]' :  'Пропущено'})
        ask_comment_6(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[использование_подвала]' : None})
        askobjecttype(message)  # Укажите, какой вопрос требуется
        return
    update_form_data(message.chat.id, {'[использование_подвала]' :  message.text})
    ask_comment_6(message)
...
def process_comment_6(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[комментарий_6]' :  'Пропущено'})
        ask_plan_match(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[комментарий_6]' : None})
        ask_basement_use_2(message)
        return
    update_form_data(message.chat.id, {'[комментарий_6]' :  message.text})
    ask_plan_match(message)
...
def process_plan_match(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[соответствие_планировки]' :  'Пропущено'})
        ask_comment_7(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[соответствие_планировки]' : None})
        ask_comment_6(message)
        return
    update_form_data(message.chat.id, {'[соответствие_планировки]' :  message.text})
    ask_comment_7(message)
...
def process_comment_7(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[комментарий_7]' :  'Пропущено'})
        ask_foundation(message)
        return
    elif message.text == 'Назад':

        update_form_data(message.chat.id, {'[комментарий_7]' : None})
        ask_plan_match(message)
        return
    update_form_data(message.chat.id, {'[комментарий_7]' :  message.text})
    ask_foundation(message)
...
def process_foundation(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[фундамент]' :  'Пропущено'})
        ask_floors(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[фундамент]' : None})
        process_comment_7(message)
        return
    update_form_data(message.chat.id, {'[фундамент]' :  message.text})
    ask_floors(message)
...
def process_floors(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[полы]' :  'Пропущено'})
        ask_load(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[полы]' : None})
        ask_foundation(message)
        return
    update_form_data(message.chat.id, {'[полы]' :  message.text})
    ask_load(message)
...
def process_load(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[нагрузка]' :  'Пропущено'})
        ask_bearing_walls(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[нагрузка]' : None})
        ask_floors(message)
        return
    update_form_data(message.chat.id, {'[нагрузка]' :  message.text})
    ask_bearing_walls(message)
...
def process_bearing_walls(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[стены]' :  'Пропущено'})
        ask_ceiling_type(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[стены]' : None})
        # Вернуться к предыдущему вопросу
        return
    update_form_data(message.chat.id, {'[стены]' :  message.text})
    ask_ceiling_type(message)
...
def process_ceiling_type(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[тип_потолка]' :  'Пропущено'})
        ask_ceiling_material(message)
        return
    elif message.text == 'Назад':

        update_form_data(message.chat.id, {'[тип_потолка]' : None})
        ask_bearing_walls(message)
        return
    update_form_data(message.chat.id, {'[тип_потолка]' :  message.text})
    ask_ceiling_material(message)
...
def process_ceiling_material(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[материал_потолка]' :  'Пропущено'})
        ask_floor_type(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[материал_потолка]' : None})
        ask_ceiling_type(message)
        return
    update_form_data(message.chat.id, {'[материал_потолка]' :  message.text})
    ask_floor_type(message)
...
def process_floor_type(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[тип_пола]' :  'Пропущено'})
        ask_floor_material(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[тип_пола]' : None})
        ask_ceiling_material(message)
        return
    update_form_data(message.chat.id, {'[тип_пола]' :  message.text})
    ask_floor_material(message)
...
def process_floor_material(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[материал_пола]' :  'Пропущено'})
        ask_roof_type(message)
        return
    elif message.text == 'Назад':

        update_form_data(message.chat.id, {'[материал_пола' : None})
        ask_floor_type(message)
        return
    update_form_data(message.chat.id, {'[материал_пола]' :  message.text})
    ask_roof_type(message)
...
def process_roof_type(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[кровля]' :  'Пропущено'})
        ask_load_roof(message)
        return
    elif message.text == 'Назад':

        update_form_data(message.chat.id, {'[кровля]' : None})
        ask_floor_material(message)
        return
    update_form_data(message.chat.id, {'[кровля]' :  message.text})
    ask_load_roof(message)
...
def process_load_roof(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[нагрузка_кровли]' :  'Пропущено'})
        ask_structure_scheme(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[нагрузка_кровли]' : None})
        ask_roof_type(message)
        return
    update_form_data(message.chat.id, {'[нагрузка_кровли]' :  message.text})
    ask_structure_scheme(message)
...
def process_structure_scheme(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[конструктивная_схема]' :  'Пропущено'})
        ask_defects(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[конструктивная_схема]' : None})
        ask_roof_type(message)
        return
    update_form_data(message.chat.id, {'[конструктивная_схема]' :  message.text})
    ask_defects(message)
...
def process_defects(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[дефекты]' :  'Пропущено'})
        ask_possibly(message)
        return
    elif message.text == 'Назад':
        update_form_data(message.chat.id, {'[дефекты]' : None})
        ask_structure_scheme(message)
        return
    update_form_data(message.chat.id, {'[дефекты]' :  message.text})
    ask_possibly(message)
...
def process_ask_possibly(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[возможность]' :  'Пропущено'})
        work_not_required(message)
        return
    elif message.text == 'Назад':

        update_form_data(message.chat.id, {'[возможность]' : None})
        ask_defects(message)
        return
    elif message.text == 'Невозможно':
        update_form_data(message.chat.id, {'[возможность]' :  message.text})
...
def process_why_impossible(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[причина_невозможности]' :  'Пропущено'})
        work_not_required(message)
        return
    elif message.text == 'Назад':

        update_form_data(message.chat.id, {'[причина_невозможности]' : None})
        ask_possibly(message)
        return
    update_form_data(message.chat.id, {'[причина_невозможности]' :  message.text})
    work_not_required(message)
...
def process_work_not_required(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[работы_не_требующие]' :  'Пропущено'})
        ask_about_nonstandard_works(message)
        return
    elif message.text == 'Назад':

        update_form_data(message.chat.id, {'[работы_не_требующие]' : None})
        ask_possibly(message)
        return
    update_form_data(message.chat.id, {'[работы_не_требующие]' :  message.text})
    ask_about_nonstandard_works(message)
...
def process_ask_about_nonstandard_works(message):
    if message.text == 'Пропустить':
        ...
        delete_non_typical_works_by_user_id(message.chat.id)
        update_form_data(message.chat.id, {'[нетиповые_работы]': 'Нет'})
        ask_for_requirements_(message)
        return
    elif message.text == 'Да':
        update_form_data(message.chat.id, {'[нетиповые_работы]': 'Да'})
...
def process_work_name(message):
    ...
    update_non_typical_works(message.chat.id, {'type' : work_name, 
                                                    'object_adress' : get_form_data(message.chat.id)['adr']})
...
#     update_non_typical_works(message.chat.id, {'period' : deadline})
...
def process_custom_deadline(message):
    ...
    update_non_typical_works(message.chat.id, {'period' : custom_deadline})
    ask_for_work_responsible(message)
...
def process_work_responsible(message):
    ...
    update_non_typical_works(message.chat.id, {'otvetstvenniy' : responsible })
...
def process_requirements_(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[требования_стандарт]' :  'Пропущено'})
        ask_for_construction_deadline(message)
        return
    elif message.text == 'Назад':
        if 'требования_стандарт' in get_form_data(message.chat.id):
            update_form_data(message.chat.id, {'[требования_стандарт]' : None})
        ask_for_more_works(message)
        return
    requirements = message.text
    update_form_data(message.chat.id, {'[требования_стандарт]' :  requirements})
    ask_for_construction_deadline(message)
...
def process_construction_deadline(message):
    if message.text == 'Пропустить':
        update_form_data(message.chat.id, {'[срок_строительства]' :  'Пропущено'})
        end_form(message)
        return
    elif message.text == 'Назад':
        if 'срок_строительства' in get_form_data(message.chat.id):
            update_form_data(message.chat.id, {'[срок_строительства]' : None})
        ask_for_requirements_(message)
        return
    construction_deadline = message.text
    update_form_data(message.chat.id, {'[срок_строительства]' :  construction_deadline})
    end_form(message)
...
def end_form(message):
    ...
    log_data_to_file(get_form_data(message.chat.id))

    file_name = get_path_to_apo(message.chat.id)
    
    insert_data_to_excel(file_name, get_form_data(message.chat.id), get_non_typical_works((message.chat.id)), message.chat.id)
...
def send_choice_message(chat_id):
    ...
    bot.send_message(chat_id, 'Выберите действие по объекту: ' + get_form_data(chat_id)['adr'] + '.', reply_markup=keyboard_2)


def get_path_to_apo(chat_id):
    objects = values.get(get_form_data(chat_id)['name'])
    adress = objects[get_form_data(chat_id)['number']]
    print(adress)
